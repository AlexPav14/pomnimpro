...
export default function CreatePage() {
  ...
  async function handleSubmit() {
    ...
    const { data, error } = await supabase.from("burials").insert({
    ...
    <div className="min-h-screen flex flex-col" style={{ background: "var(--background)" }}>
      ...
      <header style={{ background: "var(--background)", borderBottom: "1px solid var(--border)" }} className="sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <Link href="/" className="text-sm hover:opacity-70" style={{ color: "var(--muted)" }}>
            Отмена
          </Link>
        </div>
      </header>
        ...
        {step === 1 && (
          <div className="flex flex-col gap-6">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <User size={16} style={{ color: "var(--accent)" }} />
                <h1 className="text-xl font-bold">Биография</h1>
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 rounded-xl text-sm outline-none border"
                  style={{ background: "var(--surface-2)", borderColor: "var(--border)", color: form.education ? "var(--foreground)" : "var(--muted)" }}
                >
                  <option value="">Не указано</option>
                ...
                <input
                  type="text"
                  value={form.education && !["Школа","Среднее специальное","Высшее","Другое"].includes(form.education) ? form.education : ""}
                  onChange={e => set("education", e.target.value)}
          ...
          <div className="flex flex-col gap-6">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <h1 className="text-xl font-bold">Место захоронения</h1>
              </div>
        ...
        {step === 3 && (
          <div className="flex flex-col gap-6">
            <div>
              ...
              <p className="text-sm" style={{ color: "var(--muted)" }}>Обязательно добавьте два фото захоронения</p>
            </div>
        ...
        <div className="flex items-center justify-between mt-8">
          ...
          {step < STEPS.length - 1 ? (
            ...
            <button
              ...
              style={{ background: "var(--accent)", color: "white" }}
            >
              {loading ? "Сохраняем..." : <>Создать страницу <ArrowRight size={16} /></>}
            </button>
          )}
        </div>
